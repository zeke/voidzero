...
- `build-final-with-intro.mjs` builds the full looping deliverable and is the single source of truth for the final's timing. It (1) derives the outro from `intro/out/intro.mp4` — reverses it and clones the final blank frame for `OUTRO_LINGER` seconds, (re)writing the canonical `intro/out/outro.mp4`; (2) transcodes intro and outro to match the captioned segments' params (h264 High/L3.1, 1280x720/25fps, AAC 32kHz mono); (3) concat-copies intro + 12 `v5-captioned` segments + outro; (4) mixes audio: original dialogue + `data/music/pancake-parade.mp3` (slow fade-in, sidechain-ducked under dialogue, fade-out over the lingering outro) + a ringback tone (`data/sfx/originals/ringback-us.wav`) over the intro. Writes `data/final/voidzero-final-NNN.mp4`, auto-incrementing. Re-runnable, no API cost.
- All timing/levels are constants at the top of the script: `OUTRO_LINGER`; ringback `RING_CROP`/`RING_FADE_IN`/`RING_PLAY`/`RING_FADE_OUT`/`RING_GAIN`; music `MUSIC_GAIN`/`MUSIC_FADE_IN`/`MUSIC_FADE_OUT`; ducking `DUCK_THRESHOLD`/`DUCK_RATIO`/`DUCK_ATTACK`/`DUCK_RELEASE`. To reproduce the deliverable: ensure `intro/out/intro.mp4` exists (render it, see below), then `node scripts/build-final-with-intro.mjs`. The intro logo-animation timing lives in `intro/src/Composition.tsx`; everything else (outro shape, ring, music, ducking, concat order) lives in this script. Nothing about the final's timing depends on agent memory.
