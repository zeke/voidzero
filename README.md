...
1. Art. Each line gets a piece of stylized portrait art that places the speaker in a setting relevant to what they are saying. The look is illustrated and painterly rather than photorealistic, and every scene uses a different art style so the video stays visually fresh. The morning arc is the through-line: Zeke wakes up in bed, shuffles to the bathroom, pours coffee, and ends up in a sunny garden, getting visibly more awake as the news sinks in. James, meanwhile, is plugged in from the first second.

2. Voice. Each character has a cloned voice. The script is fed to a text-to-speech model so every line is spoken in the right voice, with small tweaks for delivery, like slowing Zeke down so he actually sounds half-asleep at the start.

3. Lip-sync. Each portrait is turned into a talking head, animated to match the generated speech, so the characters appear to say their lines.

4. Captions. Word-by-word captions are burned into each clip, color-matched to the art of that scene, with product names like Vite and Rolldown spelled correctly. The captions are driven by the exact script rather than guessed from the audio, so they always match what is said.

5. Stitch. All twelve captioned clips are joined into the final video.
...
This was produced in one OpenCode session. Two kinds of spend: the AI coding agent that did the work, and the Replicate models that generated the media.

Agent session (OpenCode, GPT-5.5): about $92.
...
| Replicate subtotal        |       |               | ~$17–24     |

All-in, roughly $110 to $115, most of which was the agent session, not the media. The large image count reflects the generate-many-pick-the-best approach: 446 images were generated to choose 12 final scenes.

These figures are estimates. The per-image and per-clip prices are approximate, and the agent cost is the reported session total.

## Repo layout

- `scripts/` the generation, captioning, and stitching scripts
- `transcripts/` the exact spoken lines per segment, used to drive captions
- `assets/` source reference images for the two characters
- `jamesquickfake/` James source media
- `site/` local review galleries (open with a static file server)
- `data/candidates/` every generated image candidate, by pass
- `data/audio/` generated speech and voice cache
- `data/segments/` per-segment finished clips
- `data/final/` the stitched videos, including the final captioned cut
